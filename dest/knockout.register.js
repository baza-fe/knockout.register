...
var bindingProvider = new ko.bindingProvider(); // eslint-disable-line
...
function replaceSlot(srcSlot, destSlot) {
  if (srcSlot && destSlot) {
    ...
    destSlot.parentNode.removeChild(destSlot);
  }
}
...
var customDisposeMethodName = '__custom_dispose_method__';
  ...
  loadViewModel: function loadViewModel(name, vmConfig, callback) {
    ...
    if (vmConfig && isObject(vmConfig) && isFunction(vmConfig.createViewModel)) {
      ...
      vmConfig.createViewModel = function (params, componentInfo) {
        var vm = new originalCreateViewModel(params, componentInfo); // eslint-disable-line
...
// @param {String} selector
// @param {Node} context
function getVmById(selector, context) {
